...
class ApplicationTableItem
  ...
  include DynamodbOperations::Default

  ITEM_ID_REGEX = /\A(Word|Vote|Tag)#\d{4}-\d{2}-\d{2}#.{4}\z/.freeze
  WORD_ID_REGEX = /\A(Word)#\d{4}-\d{2}-\d{2}#.{4}\z/.freeze
  TAGS_REGEX = /#.*/.freeze
  WORD_DICTIONARY_REGEX = /\A[a-z0-9\_\-]*#\d{6}#.{4}\z/.freeze
  TAG_DICTIONARY_REGEX = /\A[a-z0-9\_\-]*\z/.freeze
  ...
  def save(opts = {})
    validate!
    ApplicationTable.new.update!(attributes, opts)
  end
